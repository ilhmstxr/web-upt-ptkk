...
<head>
  ...
  <style>
    ...
    /* =======================================================
       KHUSUS HP (TIDAK MENGUBAH TABLET & DESKTOP)
       ======================================================= */
    @media (max-width: 640px) {

      /* Kecilkan gambar agar tidak ke-zoom */
      .training-card {
      ...
      .training-img {
        width: 100%;
        height: auto !important;
        object-fit: contain !important; /* tampil utuh */
      }

      /* Teks sudah aman tapi disiapkan kalau butuh */
      .card-title {
      ...
      /* biar scroll panjang tidak terbatasi */
      #scroll-pane {
...
<body class="bg-[#F1F9FC] antialiased">
  ...
  <section id="kelas-keterampilan" class="w-full bg-[#F1F9FC] py-8 md:py-10">
    <div class="max-w-7xl mx-auto px-6 md:px-12 lg:px-[80px]">
      @php($active = request('tab','keterampilan'))
        ...
        <div class="lg:col-span-8">
          <div id="scroll-pane"
            ...
            <div id="tab-ktr" class="grid grid-cols-1 gap-8">
              ...
              <figure class="group relative overflow-hidden rounded-2xl opacity-0 translate-y-4 transition-all duration-700 ease-out"
                      data-reveal>

                <div class="aspect-[4/5] md:aspect-[16/9] w-full relative overflow-hidden rounded-2xl
                            border-[2.5px] md:border-[3px] border-[#1524AF] training-card">

                  <img src="{{ asset('images/profil/' . $img) }}"
                       alt="{{ $judul }}"
                       class="w-full h-full object-cover select-none transition-transform duration-500 group-hover:scale-[1.02] training-img" />

                  <div class="absolute inset-x-0 bottom-4 md:bottom-8 px-5 md:px-8">
                    <div class="max-w-[82%] text-left">

                      <div class="relative font-[Volkhov] font-bold text-[18px] md:text-[26px] leading-tight select-none text-left card-title">
                      ...
                      <p class="mt-3 font-[Montserrat] font-medium text-[13px] md:text-[15px] leading-relaxed text-white text-justify
                                drop-shadow-[0_2px_6px_rgba(0,0,0,0.6)] card-desc">
                        {{ $desc }}
                      </p>
