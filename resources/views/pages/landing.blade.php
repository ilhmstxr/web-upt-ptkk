...
/**
 * Helper: coba ambil dari storage/public dulu,
 * kalau tidak ketemu baru fallback ke public/images.
 */
if (! function_exists('uploaded_or_asset')) {
    function uploaded_or_asset($path, $fallbackPrefix = 'images')
    {
        ...
        // Bersihkan prefix `storage/` atau `public/` kalau ada
        $normalized = preg_replace('#^(storage|public)/#i', '', trim($path));
        ...
        // Terakhir coba di public/images/...
        if (strpos($normalized, '/') !== false) {
            return asset('images/' . $normalized);
        }

        return asset("{$fallbackPrefix}/{$normalized}");
    }
}

/* ------------------ DATA HERO SLIDER (BANNER) ------------------ */

// pastikan selalu punya collection (minimal kosong)
$banners = Banner::query()
    ->where('is_active', true)
    ->orderBy('sort_order', 'asc')
    ->get();

// kalau kosong, nanti dipakai fallback 3 slide statis
...
<section class="relative bg-[#F1F9FC] py-6 md:py-10">
      ...
      @if($latestBeritas->isEmpty())
        ...
        @for($i=1;$i<=3;$i++)
          <article class="group bg-white border border-[#B6BBE6] rounded-2xl shadow-sm p-4 transition-all duration-300 hover:border-[#1524AF] hover:shadow-md">
            <div class="w-full h-[160px] bg-[#E7ECF3] rounded-lg mb-4 flex items-center justify-center text-sm text-[#727272]">
              Belum ada berita
            </div>
  ...
  @php
      // gambar berita: samakan dengan halaman berita/show
      $img = $b->image;
      ...
      if ($img) {
          ...
          if (\Illuminate\Support\Str::startsWith($img, ['http://', 'https://'])) {
              ...
              // kalau disimpan "berita/xxx.jpg" di disk public
      ...
      $pubDate = $b->published_at
          ? $b->published_at->format('d F Y')
          : ($b->created_at ? $b->created_at->format('d F Y') : now()->format('d F Y'));
  @endphp
          ...
          <article class="group bg-white border border-[#B6BBE6] rounded-2xl shadow-sm p-4 transition-all duration-300 hover:border-[#1524AF] hover:shadow-md">
            ...
            <a href="{{ route('berita.show', $b->slug ?? $b->id) ?? '#' }}"
               class="text-[#595959] group-hover:text-[#1524AF] text-sm font-medium inline-flex items-center gap-1 transition-colors duration-300">
              Baca Selengkapnya →
            </a>
          </article>
        @endforeach
  ...
  <section class="relative bg-[#F1F9FC] py-4 md:py-6">
    ...
    {{-- SCRIPT sorotan (sama seperti sebelumnya) --}}
    <script>
      (function(){
  ...
  {{-- ... bagian Kompetensi Pelatihan, Data Statistik, Panduan Pelatihan, Footer (sama seperti sebelumnya) --}}
    ...
    (function(){
      ...
      new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            { label: 'Pre-Test', data: [8, 22, 12, 29], borderColor: '#FF6107', pointBackgroundColor: '#FF6107', pointBorderColor: '#FF6107', borderWidth: 2, tension: 0.35, pointRadius: 4, pointHoverRadius: 5, fill: false },
            { label: 'Post-Test', data: [24, 53, 75, 94], borderColor: '#2F4BFF', pointBackgroundColor: '#2F4BFF', pointBorderColor: '#2F4BFF', borderWidth: 2, tension: 0.35, pointRadius: 4, pointHoverRadius: 5, fill: false },
            { label: 'Praktek', data: [38, 70, 35, 60], borderColor: '#6B2C47', pointBackgroundColor: '#6B2C47', pointBorderColor: '#6B2C47', borderWidth: 2, tension: 0.35, pointRadius: 4, pointHoverRadius: 5, fill: false },
            { label: 'Rata-Rata', data: [52, 10, 26, 49], borderColor: '#DBCC8F', pointBackgroundColor: '#DBCC8F', pointBorderColor: '#DBCC8F', borderWidth: 2, tension: 0.35, pointRadius: 4, pointHoverRadius: 5, fill: false }
          ]
        },
        options: {
          maintainAspectRatio: false,
          layout: { padding: { top: 10, right: 16, left: 16, bottom: 8 } },
          plugins: { legend: { position: 'top', align: 'center', labels: { usePointStyle: true, pointStyle: 'circle', boxWidth: 10, boxHeight: 10, padding: 12, color: '#081526', font: { size: 13, weight: '600' } } }, tooltip: { callbacks: { label: (ctx) => ${ctx.dataset.label}: ${ctx.formattedValue} } } },
          scales: { x: { offset: true, ticks: { font: { size: 12 }, color: '#8787A3' }, grid: { display: true, drawTicks: false, color: '#8787A3', lineWidth: 1 }, border: { display: true, color: '#8787A3', width: 1.5 } }, y: { beginAtZero: true, min: 0, max: 100, ticks: { stepSize: 20, font: { size: 12 }, color: '#8787A3' }, grid: { color: (ctx) => (ctx.tick.value === 0 ? '#8787A3' : 'transparent'), lineWidth: (ctx) => (ctx.tick.value === 0 ? 1.5 : 0), drawTicks: false }, border: { display: true, color: '#8787A3', width: 1.5 } } }
      });
    })();
  </script>
