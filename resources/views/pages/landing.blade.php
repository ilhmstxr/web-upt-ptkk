...
/* Ambil banners dari model. Ambil yang aktif dan urut berdasarkan sort_order. */
...
/**
 * Helper kecil di sini hanya untuk mengubah path ke URL publik storage bila file ada di storage/app/public,
 * kalau tidak ada fallback ke asset('images/...')
 *
 * NOTE: Kita memakai Storage::exists('public/...') dan Storage::url('...') sehingga URL akhir menjadi /storage/...
 */
function storage_or_asset($publicPath, $fallbackAsset)
{
    // $publicPath contoh: 'berita/01.jpg' atau 'banners/01.jpg'
    if ($publicPath && Storage::exists('public/' . ltrim($publicPath, '/'))) {
        // Storage::url('berita/01.jpg') -> /storage/berita/01.jpg
        return Storage::url(ltrim($publicPath, '/'));
    }
        ...
        <div id="hero-track" class="flex items-center gap-4 md:gap-6 overflow-x-auto snap-x snap-mandatory scroll-smooth no-scrollbar select-none py-8" style="scrollbar-width:none;-ms-overflow-style:none;">
          ...
          @if($banners->isNotEmpty())
            @for($i = $cloneCount; $i > 0; $i--)
              @php $idx = ($realCount - $i) % $realCount; $b = $banners->values()[$idx];
                   // coba cari file di storage/public/banners/...
                   $bannerSrc = storage_or_asset('banners/' . basename($b->image ?? ''), 'images/beranda/slide1.jpg');
              @endphp
            ...
            @for($i = 0; $i < $cloneCount; $i++)
              @php $idx = $i % $realCount; $b = $banners->values()[$idx];
                   $bannerSrc = storage_or_asset('banners/' . basename($b->image ?? ''), 'images/beranda/slide1.jpg');
              @endphp
              <div class="hero-slide clone shrink-0 snap-center w-[70%] md:w-[76%] lg:w-[80%] transition-transform duration-300" data-real="{{ $idx }}">
                <div class="w-full h-[46vw] md:h-[420px] lg:h-[520px] max-h-[560px] min-h-[220px] rounded-2xl border-[2.5px] md:border-[3px] border-[#1524AF] overflow-hidden">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16 items-start md:items-center">
        ...
        <div class="w-full flex justify-center md:justify-start md:pl-2 lg:pl-4">
          <div class="rounded-2xl overflow-hidden shadow-xl ring-2 ring-[#1524AF] max-w-[420px] md:max-w-[480px] lg:max-w-[520px]">
            @php
              // cek storage terlebih dahulu, fallback ke public images jika tidak ada
              $ceritaSrc = storage_or_asset('profil/cerita-kami.svg', 'images/cerita-kami.svg');
            @endphp
            <img src="{{ $ceritaSrc }}"
        ...
        <div class="relative bg-[#DBE7F7] rounded-full overflow-hidden h-[54px] md:h-[58px] lg:h-[62px] flex items-center">
          ...
          <div class="relative w-full overflow-hidden">
            ...
            <div class="jatim-marquee flex w-[200%] items-center animate-[jatim-scroll-x_linear_infinite] [animation-duration:24s]">
              {{-- icons memakai storage jika ada, fallback ke public images --}}
              <img src="{{ Storage::exists('public/icons/cetar.svg') ? Storage::url('icons/cetar.svg') : asset('images/icons/cetar.svg') }}" class="h-[26px] md:h-[32px] lg:h-[42px] flex-shrink-0" alt="Cetar">
              <img src="{{ Storage::exists('public/icons/dindik.svg') ? Storage::url('icons/dindik.svg') : asset('images/icons/dindik.svg') }}" class="h-[26px] md:h-[32px] lg:h-[42px] flex-shrink-0" alt="Dindik">
              <img src="{{ Storage::exists('public/icons/jatim.svg') ? Storage::url('icons/jatim.svg') : asset('images/icons/jatim.svg') }}" class="h-[26px] md:h-[32px] lg:h-[42px] flex-shrink-0" alt="Jatim">
              <img src="{{ Storage::exists('public/icons/berakhlak.svg') ? Storage::url('icons/berakhlak.svg') : asset('images/icons/berakhlak.svg') }}" class="h-[26px] md:h-[32px] lg:h-[42px] flex-shrink-0" alt="Berakhlak">
              <img src="{{ Storage::exists('public/icons/optimis.svg') ? Storage::url('icons/optimis.svg') : asset('images/icons/optimis.svg') }}" class="h-[26px] md:h-[32px] lg:h-[42px] flex-shrink-0" alt="Optimis">

              <div class="flex w-1/2 items-center justify-between px-6 md:px-10 lg:px-16 gap-4 md:gap-6 lg:gap-8" aria-hidden="true">
                <img src="{{ Storage::exists('public/icons/cetar.svg') ? Storage::url('icons/cetar.svg') : asset('images/icons/cetar.svg') }}" class="h-[26px] md:h-[32px] lg:h-[42px] flex-shrink-0" alt="">
                <img src="{{ Storage::exists('public/icons/dindik.svg') ? Storage::url('icons/dindik.svg') : asset('images/icons/dindik.svg') }}" class="h-[26px] md:h-[32px] lg:h-[42px] flex-shrink-0" alt="">
                <img src="{{ Storage::exists('public/icons/jatim.svg') ? Storage::url('icons/jatim.svg') : asset('images/icons/jatim.svg') }}" class="h-[26px] md:h-[32px] lg:h-[42px] flex-shrink-0" alt="">
                <img src="{{ Storage::exists('public/icons/berakhlak.svg') ? Storage::url('icons/berakhlak.svg') : asset('images/icons/berakhlak.svg') }}" class="h-[26px] md:h-[32px] lg:h-[42px] flex-shrink-0" alt="">
                <img src="{{ Storage::exists('public/icons/optimis.svg') ? Storage::url('icons/optimis.svg') : asset('images/icons/optimis.svg') }}" class="h-[26px] md:h-[32px] lg:h-[42px] flex-shrink-0" alt="">
              </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          @for($i=1;$i<=3;$i++)
            <article class="group bg-white border border-[#B6BBE6] rounded-2xl shadow-sm p-4 transition-all duration-300 hover:border-[#1524AF] hover:shadow-md">
              ...
              <p class="text-sm text-[#081526] mb-3 leading-relaxed">
                Silakan tambahkan berita melalui panel admin.
              </p>
        ...
        @else
          @foreach($latestBeritas as $b)
            @php
              // Normalisasi thumbnail: cek storage/public/berita/..., fallback ke placeholder di public/images/berita/placeholder.jpg
              $imgCandidate = $b->image ? basename($b->image) : null;
              $imgUrl = $imgCandidate ? (Storage::exists('public/berita/' . $imgCandidate) ? Storage::url('berita/' . $imgCandidate) : asset('images/berita/placeholder.jpg')) : asset('images/berita/placeholder.jpg');
