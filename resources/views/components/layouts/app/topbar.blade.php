...
<div class="w-full bg-[#F1F9FC]/90 backdrop-blur-sm relative z-[60]" style="font-family: 'Montserrat', sans-serif;">
 <div class="max-w-7xl mx-auto px-6 md:px-12 lg:px-[80px] py-2 flex items-center justify-between">
    ...
    <div class="relative inline-block text-left">
      <button id="langBtn" type="button"
        ...
        <span id="currentLang">Indonesia</span>
      ...
      <div id="langMenu"
        ...
        <a href="#" class="lang-option block px-3 py-1.5 hover:bg-slate-50 rounded-t-lg">Indonesia</a>
        <a href="#" class="lang-option block px-3 py-1.5 hover:bg-slate-50 rounded-b-lg">English</a>
      </div>
    </div>

    {{-- Ikon Sosmed --}}
    <div class="flex items-center gap-2 sm:gap-3">
      <a href="https://www.instagram.com/uptptkk.jatim/" class="text-slate-700 hover:text-pink-600 transition-colors duration-200 flex-shrink-0" aria-label="Instagram" target="_blank">
      ...
      <a href="https://www.youtube.com/@uptptkkjatim1707" class="text-slate-700 hover:text-red-600 transition-colors duration-200 flex-shrink-0" aria-label="YouTube" target="_blank">
        <svg class="w-3.5 h-3.5 sm:w-4 sm:h-4 lg:w-[18px] lg:h-[18px]" fill="currentColor" viewBox="0 0 24 24">
...
<script>
  ...
  const currentLang = document.getElementById('currentLang'); // Ambil span teks
  const langOptions = document.querySelectorAll('.lang-option'); // Ambil pilihan bahasa
  ...
  if (langBtn && langMenu) {
    ...
    langOptions.forEach(option => {
      option.addEventListener('click', (e) => {
        e.preventDefault(); // Mencegah reload halaman (karena href="#")
        
        // Ambil teks dari opsi yang diklik (Indonesia/English)
        ...
        // --- CATATAN ---
        // Di sini kamu bisa tambahkan logika redirect ke route Laravel 
        // jika ingin bahasa websitenya benar-benar berubah.
        // Contoh: window.location.href = '/lang/' + (selectedText === 'English' ? 'en' : 'id');
      });
    });
  }
</script>
