...
@section('content')
    <div class="bg-white p-6 rounded-xl shadow-md fade-in">
        ...
        @if (isset($pertanyaan))
            ...
            {{-- Form Jawaban --}}
            <form id="form-tes" action="{{ route('dashboard.pretest.submit', ['percobaan' => $percobaan->id]) }}"
                method="POST">
                @csrf
                ...
                <div class="mb-4 flex flex-wrap gap-2">
                    @foreach ($pertanyaanList as $p)
                        ...
                        @endphp
                        <button type="submit" name="next_q" value="{{ $loop->index }}"
                    ...
                    @if ($pertanyaan->opsiJawabans && $pertanyaan->opsiJawabans->count() > 0)
                        @foreach ($pertanyaan->opsiJawabans as $opsi)
                            ...
                            <label class="block p-2 border rounded hover:bg-gray-100 cursor-pointer">
                                <input type="radio" name="jawaban[{{ $pertanyaan->id }}]" value="{{ $opsi->id }}"
                                    class="mr-2" {{ $checked ? 'checked' : '' }} required>
                ...
                <div class="flex justify-between">
                    @if ($currentQuestionIndex > 0)
                        <button type="submit" name="next_q" value="{{ $currentQuestionIndex - 1 }}"
