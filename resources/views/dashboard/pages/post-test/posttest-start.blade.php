...
@section('page-title', 'Post-Test: Soal {{ $currentQuestion ?? 1 }}')

@section('content')
<div class="bg-white p-6 rounded-xl shadow-md fade-in">
    @if(isset($pertanyaan))
        <form action="{{ route('dashboard.posttest.submit', $percobaan->id) }}" method="POST">
            @csrf
    ...
    @else
        ...
        <a href="{{ route('dashboard.posttest.result', $percobaan->id) }}" 
