...
@section('page-title', 'Monev: Soal '.(($currentQuestionIndex ?? 0) + 1))

@section('content')
<div class="bg-white p-6 rounded-2xl shadow-md fade-in max-w-4xl mx-auto">

    @php($hasQuestion = isset($pertanyaan) && $pertanyaan)

    @if($hasQuestion)

        @php($jawabanCollection = $percobaan->jawabanUser ?? collect())
        @php($totalSoal = $pertanyaanList->count())
        @php($terjawab = $jawabanCollection->count())
        @php($progress = $totalSoal > 0 ? round(($terjawab / $totalSoal) * 100, 2) : 0)

        @php($existing = $jawabanCollection->firstWhere('pertanyaan_id', $pertanyaan->id))
        @php($existingNilai = $existing->nilai_jawaban ?? null)
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-5">
            <div class="text-sm text-slate-600">
                Soal <b>{{ ($currentQuestionIndex ?? 0) + 1 }}</b> dari {{ $totalSoal }}
            </div>
        ...
        >
            ...
            <div class="flex flex-wrap gap-2">
                @foreach($pertanyaanList as $idx => $p)
                    @php($answered = $jawabanCollection->contains('pertanyaan_id', $p->id))
                    @php($isActive = $idx === ($currentQuestionIndex ?? 0))

                    @php($btnClass = $isActive
            ...
            <input type="hidden" id="next_q_input" name="next_q" value="{{ ($currentQuestionIndex ?? 0) + 1 }}">
            ...
            <div class="flex items-center justify-between pt-2 border-t border-slate-100">
                @if(($currentQuestionIndex ?? 0) > 0)
                    <button
                        type="button"
                        onclick="goToQuestion({{ ($currentQuestionIndex ?? 0) - 1 }})"
                ...
                >
                    {{ ((($currentQuestionIndex ?? 0) + 1) == $totalSoal) ? 'Selesai ✓' : 'Selanjutnya →' }}
                </button>
            </div>
        </form>
